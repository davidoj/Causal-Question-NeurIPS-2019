...
and any See-Do model $\kernel{T}:\Theta\times D\to \Delta(\mathcal{E}\otimes \mathcal{F})$ there exists a set of auxhiliary conditions $A$ such that the model $(\diagram{I},A)$ is equivalent to $\kernel{T}$\todo{subject to irrelevance of index $\Theta$}. To illustrate how influence diagrams and string diagrams compare, $\kernel{T}$ can be drawn:
...
There is always some $\kernel{H}:\Theta\to\Delta(\sigalg{E})$ and $\kernel{C}:\Theta\times D\to \Delta(\sigalg{E}\otimes\sigalg{F})$ such that: $\seedo_\theta$ is equal to
\begin{align}
