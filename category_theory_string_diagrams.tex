...
Given $\kappa:D\to X\times Y$, a kernel $c:D\times X\to Y$ is a \emph{generalised disintegration} (``g-disintegration'') of $\kappa$ if the following holds:
...
\begin{theorem}
For all $\kappa:D\to X\times Y$, if $D$ is countable and $X\times Y$ is standard measurable, a g-disintegration of $\kappa$ exists.
\end{theorem}
