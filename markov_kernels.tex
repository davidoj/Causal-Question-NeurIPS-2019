...
\begin{lemma}[Label assignments exist]
...
\todo[inline]{This definition is unfinished, I'm tired of terminology}

\begin{definition}[Wire labels -- disintegration]\label{def:wl_disint}
...
As an aside, \citet{hajek_what_2003} pointed out that in general there are many Markov kernels that satisfy the definition of conditional probability for a given probability measure and random variables. For a given Markov kernel space $(\kernel{K},\Omega,\mathcal{F},D,\mathcal{D})$ there is in general no probability measure on $\Omega\times D$ such that $\kernel{K}$ is uniquely defined as a disintegration of $\mu$. However, by limiting $D$ to countable sets there is for every Markov kernel space $(\kernel{K},\Omega,D)$ a probability space $(\prob{P},\Omega\times D)$ such that $\kernel{K}$ is the unique disintegraion of $\prob{P}$ on well-chosen random variables.
