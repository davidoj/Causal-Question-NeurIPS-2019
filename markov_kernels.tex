...
Given a sample space $\Omega$ and an input space $D$, a random variable $\RV{X}$ is a measurable function $\Omega\times D\to E$ for arbitrary measurable $E$. If there is no decision space, it is simply a measurable function $\Omega\to X$.
...
Given a conditional probability space $(\mathscr{K},\Omega,D)$, random variables $\RV{X}$ and $\RV{Y}$ and kernel $\mathscr{K}_{\RV{X},\RV{Y}}=:\mathbf{L}:D\to \Delta(\mathcal{E}\otimes\mathcal{F})$, we say that $\mathbf{M}:E\to \Delta(\mathcal{F})$ is a disintegration of $\mathbf{L}$ if
...
Similarly, we say $\mathbf{M}\in \mathscr{K}_{\RV{Y}|\RV{X}}$. Note that unlike disintegrations of probability measures, we can't assume that disintegrations of kernels exist.

\end{definition}

\begin{definition}[Wire labels - disintegrations]
Given a conditional probability space with ambient kernel $\mathscr{K}:D\to \Delta(\mathcal{F})$ (or a probability space with measure $\mathbb{P}$), define $\mathscr{K}^*$ to be the kernel
\begin{align}
\begin{tikzpicture}
\path (0,0) node (O) {}
++(0.5,0) coordinate (copy0)
...
On the other hand, we can't in general rely on the existence of 

\begin{align}
\begin{tikzpicture}
\path (0,0) node (O) {$\RV{X}$}
++ (0.7,0) node[kernel] (m) {$\mathbf{N}$}
++ (0.7,0) node (E) {$\RV{Y}$};
\draw (O) -- (m) -- (E);
\end{tikzpicture}
\end{align}
...
Disintegrations of general Markov kernels, in contrast, usually \emph{do not} exist. Take $\mathbf{L}:\{0,1\}\to \Delta(\{0,1\}^2)$ where $\mathbf{L}(1;[x,y])=\delta_y(x)$ and $\mathbf{L}(0;[x,y])=\delta_{(1-y)}(x)$. Clearly there is no disintegration of $\mathbf{L}$. We can guarantee their existence under a number of strong assumptions (probably stronger than necessary).
...
In general, we can only be confident that a disintegration of a Markov kernel exists if said disintegration is conditioned on the input to the original kernel. Nonetheless, it is helpful to have a notion of more general disintegrations - we might, for example, want to define a conditional probability space such that certain other disintegrations do in fact exist.

With these definitions in place, we can move on to our definition of wire labels.





\begin{definition}[Implicit conditional probability space]
...
It is notable that in a probability space, the following property does \emph{not} generally hold:
\begin{align}
\mathrm{Sat:}\begin{tikzpicture}
\path (0,0) node (X) {$\RV{Z}$}
++ (0.7,0) node[kernel] (K) {$\mathbf{K}$}
...
Consider $\RV{Z}=\RV{X}$ and $\mathbf{K}=z\mapsto \mathrm{Bernouli}(0.5)$ for all $z$. Then $\mathbf{L}=x\mapsto \mathbb{P}_{\RV{X}}$ and $\mathbf{KL}=z\mapsto \mathbb{P}_{\RV{X}}$ but $\mathbb{P}_{\RV{X}|\RV{Z}} = z\mapsto \delta_z\neq \mathbf{KL}$.
