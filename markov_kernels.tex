...
There is a clear graphical intuition in which the wire labeled $\RV{Y}$ represents the ``same thing'' in the left and the right diagram, and similarly, for arbitrary $\mathbf{A}$, $\RV{Z}$ usually does \emph{not} represent the ``same thing'' as $\RV{X}$. This intuition is supported by the fact that the marginal distribution on the wire labeled $\RV{Y}$ is the same in both diagrams while the wires $\RV{X}$ and $\RV{Z}$ will usually not have the same marginal distributions. Just as we can refer to a random variable without referring to a particular joint distribution, it would be handy to refer to the wire named $\RV{Y}$ without also referring to a specific diagram. 
