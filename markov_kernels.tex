...
We introduce \emph{wire names} in order to refer to particular wires in a diagram. It is not sufficient to label wires with spaces, as wires that share a space may not be interchangeable. It is not hard to imagine a probability distribution $\mu\in \Delta(\{0,1\}^2)$ where in general $\mu(A\times B)\neq \mu(B\times A)$ for $A,B\in \Delta(\{0,1\})$.
